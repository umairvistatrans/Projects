...
publicWidget.registry.return_calander = publicWidget.Widget.extend({
    selector: '#calendar-div',
    events: {
    ...
    init() {
       ...
       start: function () {
            return this._super(...arguments).then(async () => {
                ...
                if (urlParams.get('filterby') === 'custom') {
                    const listBtnCalendar = $(".list-btn-calendar");
                    if (listBtnCalendar.length) {
                        listBtnCalendar.click();
                        const dateBegin = urlParams.get('date_begin');
                        const element = $(`#${dateBegin}`);
                        if (element.length) {
                            element.addClass('o_slot_selected active');
                        }
                    }
                }
           });
        ...
        _onClickDaySlot: function (ev) {
            ...
            window.location.href = "/my/visits?filterby=custom&date_begin=" + slotDate
