...
class OdooAPI(http.Controller):
    ...
    @http.route(
        '/api/product/updates',
        type='http', auth="user", methods=['GET'], csrf=False)
    def get_product_model_records(self, **post):
        products = request.env['product.product'].search([('product_qty_updated', '=', True)])
        list_of_products = {'result':[]}
        for item in products:
            list_of_products['result'].append({
                 'product_id': item.id,
                'sku': item.default_code,
                'qty': item.free_per_location,
            })
        products.sudo().write({'product_qty_updated':False})
        return http.Response(
            json.dumps(list_of_products),
            status=200,
            mimetype='application/json'
        )
