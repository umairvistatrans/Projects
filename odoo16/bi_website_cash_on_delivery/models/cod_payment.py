...
class CODAcquirer(models.Model):
	...
	def _get_compatible_providers(
			...
			is_express_checkout=False, is_validation=False, website_id=None, sale_order_id=None, **kwargs
	):
		res = super(CODAcquirer, self)._get_compatible_providers(company_id, partner_id, amount,
																 currency_id=None, force_tokenization=False,
																 is_express_checkout=False, is_validation=False,
																 website_id=website_id,
																 sale_order_id=sale_order_id, **kwargs)
		sale_order = self.env['sale.order'].browse(sale_order_id)
		if sale_order and any(line.is_preorder for line in sale_order.order_line):
			res = res.filtered(lambda p: p.code != 'cod')
		return res
