...
class IrWhatsappServer(models.Model):
    ...
    allowed_user_ids = fields.Many2many('res.users', 'allowed_user_ids_rel', string="Allowed Users",
                                        domain=[('share', '=', False)])
    notify_user_ids = fields.Many2many('res.users', 'notify_user_ids_rel', string="Channel Notification",
    ...
    def _get_mail_message_whatsapp(self):
        # wht_config = self.filtered(lambda config: config.status == 'authenticated')
        for was in self:
            if was.status == 'authenticated':
                KlikApi = was.klikapi()
                KlikApi.auth()
    ...
    def _formatting_mobile_number(self, number):
        for rec in self:
            module_rec = self.env['ir.module.module'].sudo().search_count([
                ...
                ('state', '=', 'installed')])
            country_code = str(
                rec.partner_id.country_id.phone_code)  # if rec.partner_id.country_id else str(self.company_id.country_id.phone_code)
...
A QR code is valid only for 45 seconds. Message sennding will be available right after authorization."""
    ...
    def _process_messages(self, value):
        ...
        kwargs = {
            ...
            'author_id': self.env['res.partner'].search([('whatsapp', '=', sender_mobile)], limit=1).id,
