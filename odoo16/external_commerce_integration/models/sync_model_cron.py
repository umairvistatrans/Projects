...
class SyncModelsTable(models.Model):
    _name = 'res.sync.table'
    ...
    def website_sync_set_product(self):
        ...
        _logger.info('SKUS {}'.format(product_skus))
        for product in self.env['product.product'].sudo().search([]):
            if product.default_code not in product_skus:
                payload = {'name': product.name,
                           ...
                           'manage_stock': '1',
                           'qty': str(product.free_per_location), # removed qty_available
                           'in_stock': '0' if not product.free_per_location else '1', # removed qty_available
                           }
    ...
    def get_all_customers(self):
        ...
        start_date = self.get_config_params().last_sync_date if self.get_config_params().last_sync_date else str(datetime.now().date() - timedelta(days=10))
        end_date = str(datetime.now().date() + timedelta(days=1))
        payload = {'from': start_date, 'to': end_date}
        ...
        for page in range(1, last_page + 1):
            ...
            for record in response['data']:
                ...
                if not is_exist:
                    ...
                    _logger.info('Brand Already exist with name  %s' % is_exist.name)
                    is_exist.write({
                        'name': record['name'],
...
#         start_date =str(datetime.strptime(str(datetime.now().date()), DEFAULT_SERVER_DATE_FORMAT)- timedelta(days=3))
        ...
        for page in range(1, last_page + 1):
            ...
            for record in response['data']:
                if record.get('sku'):
                    ...
                    _logger.info('Product Data {}'.format(is_exist))
                    if not is_exist:
                        _logger.info('Data from API {}'.format(record))
                        create_obj = {
                        ...
                        if record.get('heavy_shipping_enabled') == 1:
                            create_obj['price_type'] = 'h'
                            create_obj['price_val'] = float(35)
                        if record.get('lite_shipping_enabled') == 1:
                            create_obj['price_type'] = 'l'
                            create_obj['price_val'] = float(15)
                        _logger.info('Product Dict {}'.format(create_obj))
                        is_exist = self.env['product.template'].sudo().create(create_obj)
                        _logger.info('Product Data {}'.format(is_exist))
                        ...
                        _logger.info('Product Already exist with external id %s' % is_exist.external_id)
                        update_obj = {
                        ...
                        if record.get('heavy_shipping_enabled') == 1:
                            update_obj['price_type'] = 'h'
                            update_obj['price_val'] = float(35)
                        if record.get('lite_shipping_enabled') == 1:
                            update_obj['price_type'] = 'l'
                            update_obj['price_val'] = float(15)
                        is_exist.write(update_obj)
        self.website_auth_logout_token(api_token)
        return True
    ...
    def website_sync_get_customers(self):
        ...
        api_token = self.website_auth_token()
        customer_url = '{0}{1}?api_token={2}'.format(url, '/api/v1/odoo/customers', api_token)
        start_date = self.get_config_params().last_sync_date if self.get_config_params().last_sync_date else str(datetime.now().date() - timedelta(days=6))
        # start_date =str(datetime.strptime(start_date, DEFAULT_SERVER_DATE_FORMAT)- timedelta(days=6))
        end_date = str(datetime.now().date() + timedelta(days=1))
    ...
    def website_sync_get_orders(self):
        ...
        if response.status_code == 200:
            ...
            for page in range(1, last_page + 1):
                ...
                _logger.info('Test Request Response | {}'.format(response))
                if response.status_code == 200:
                    response = response.json()
                    for record in response.get('data'):
                        if record['status']=='order_submitted':
                            ...
                            if not sale_exist:
                                ...
                                if record.get('payment_method_service_cost'):
                                    product= self.env.ref('external_commerce_integration.product_product_payment_charge')
                                    sale_lines.append({
                                        'name':'Payment Charge',
                                        'product_id': product.id,
                                        'product_uom_qty': 1,
                                        'product_uom': product.uom_id.id,
                                        'price_unit': float(record['payment_method_service_cost']['amount']),
                                    })
                                ...
                                _logger.info('Sale order already exist with external id %s' % sale_exist.external_id)
                else:
    ...
    def sync_orders(self):
        _logger.info('company details {}'.format(self.get_config_params().read()))
        _logger.info('updating from cronnnnnnnnnnnnnnnnnnnnnnnn')
        self.website_sync_get_orders()
        self.get_config_params().sudo().write({
            'last_sync_date': str(datetime.now().date())
        })
        _logger.info('company details %s' % self.get_config_params().read())
