...
class BarcodeLabels(models.TransientModel):
    ...
    def default_get(self, fields):
                    ...
                    if move.move_line_ids:
                        for moveline in move.move_line_ids:
                            if moveline.qty_done > 0:
                                order_qty = int(abs(moveline.qty_done)) or 1.0
                                product_get_ids += [(0, 0, {
                                    'product_id': moveline.product_id.id,
                                    'qty': order_qty,
                                })]
                    else:
                        if move.product_id and move.product_id.type != 'service':
                            order_qty = int(abs(move.product_uom_qty)) or 1.0
                            product_get_ids += [(0, 0, {
        ...
        if not view_id.arch:
            raise UserError('Someone has deleted the reference '
    ...
    def _create_paper_format(self, data):
        report_action_id = self.env['ir.actions.report'].sudo().search(
            [('report_name', '=', 'dynamic_barcode_labels.report_barcode_labels')])
        if not report_action_id:
            raise UserError('Someone has deleted the reference view of report, Please Update the module!')
        config_rec = self.env['barcode.configuration'].search([], limit=1)
        if not config_rec:
            raise UserError(_(" Please configure barcode data from "
                              "configuration menu"))
    ...
    def print_report(self):
        if not self.env.user.has_group('dynamic_barcode_labels.group_barcode_labels'):
            raise UserError(_("You have not enough rights to access this "
                              "document.\n Please contact administrator to access "
                              "this document."))
        if not self.product_get_ids:
            raise UserError(_(""" There is no product lines to print."""))
        config_rec = self.env['barcode.configuration'].search([], limit=1)
        if not config_rec:
            raise UserError(_(" Please configure barcode data from "
                              "configuration menu"))
        ...
        for product in browse_pro:
            ...
            if not barcode_value:
                raise UserError(_('Please define barcode for %s!' % (product['name'])))
            try:
                ...
                raise UserError('Select valid barcode type according barcode field value or check value in field!')
