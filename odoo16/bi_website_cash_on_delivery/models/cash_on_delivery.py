...
class CreditPaymentAcquirer(models.Model):
    ...
    cod_config = fields.Many2one('cod.config', 'Cash on Delivery Configuration')
...
class CODPaymentCollection(models.Model):
    ...
    def done_collection(self):
        ...
        if self.transaction_id:
            self.transaction_id.update({'state': 'done'})

        if self.sale_order_id:
            # Mark the delivery as done
            pickings = self.sale_order_id.picking_ids.filtered(lambda p: p.state not in ('done', 'cancel'))
    ...
    def cancel_collection(self):
        ...
        self.transaction_id.update({'state': 'cancel'})
...
class Website(models.Model):
    ...
    def get_all_zip(self):
        ...
        if curr_pay_method.cod_config:
            if (not curr_pay_method.available_country_ids) or (
                    user.country_id in curr_pay_method.available_country_ids):
                if curr_pay_method.cod_config.cod_zip or curr_pay_method.cod_config.cod_state:
                    if curr_pay_method.cod_config.cod_zip and curr_pay_method.cod_config.cod_state:
                        if user.zip in [i.zip_code for i in
                                        curr_pay_method.cod_config.cod_zip] and user.state_id in curr_pay_method.cod_config.cod_state:
                            count = 1
